...
calculate_features <- function(mytable, fps, feature_calculation_temporal_window_size){
  ...
  #### Output table model that will be used to store calculated data for later output to ARFF file
  output_table <- data.table(
    ...
    "mean_gaze_fixation_time_secs" = 0.0, "max_gaze_fixation_time_secs" = 0.0, "gaze_fixations_count" = 0.0,
    
    "gaze_approach_time_ratio" = 0.0, "min_gaze_approach_time_secs" = 0.0, "median_gaze_approach_time_secs" = 0.0,
    "mean_gaze_approach_time_secs" = 0.0, "max_gaze_approach_time_secs" = 0.0, "gaze_approaches_count" = 0.0,
  ...
  for(j in 1:dim(output_table)[1]) {
    ...
    if(is.null(eye_fixation_frames)){
      ...
      output_table[j,"gaze_fixations_count"] <- 0.0
    } else {
      ...
      output_table[j,"gaze_fixations_count"] <- length(eye_fixation_frames)
    }
    frame_count <- 0
    eye_fixation_frames <- {}
    ...
    if(is.null(eye_gaze_approach_frames)){
      output_table[j,"min_gaze_approach_time_secs"] <- 0.0
      ...
      output_table[j,"gaze_approaches_count"] <- 0.0
    } else {
      output_table[j,"min_gaze_approach_time_secs"] <- min(eye_gaze_approach_frames) * frame_time_in_seconds
      ...
      output_table[j,"gaze_approaches_count"] <- length(eye_gaze_approach_frames)
    }
    frame_count <- 0
    eye_gaze_approach_frames <- {}
